...
int main(void)
{
	...
	xTaskCreate(vLED_Task_handler,"Led Handler",500,NULL,1,&xTaskHandle1);

	xTaskCreate(vButton_Task_handler,"Button Handler",500,NULL,1,&xTaskHandle2);
...
void vLED_Task_handler(void *param)
{
	while(1)
	{
		...
		//lets wait until receiving any notification event from button_Taks

		if(xTaskNotifyWait(0,0,&current_notification_value,portMAX_DELAY) == pdTRUE)
		{
			//WE received the notification. LED toggle
			GPIO_ToggleBits(GPIOD,GPIO_Pin_12);
			sprintf(usg_msg,"Notification is received : Button press count : %ld \r\n",current_notification_value);
			printmsg(usg_msg);
		}
...
void vButton_Task_handler(void *param)
{
	while(1)
	{
		if(GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0))
		{
			...
			//Compensate for the button debouncing
			rtos_delay(100);
